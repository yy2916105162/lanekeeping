...
#(t, theta(t)) is stored in 'sol.t' and 'sol.y[2]'. Both are numpy arrays
times = sol.t
theta_trajectory = sol.y[2]

#plot the figure of theta(t) again time
plt . plot(times, theta_trajectory.T)
plt.xlabel('Time (s)')
plt.ylabel('theta (rad/s)')
plt.title('Trajectory of theta(t)')
plt.grid()
plt . show()